...
def calculate_aqi_500(pm25):
    """Standard US-EPA AQI Calculation (0-500 scale)"""
    if pm25 <= 12.0:
...
def fetch_and_push():
    for city in CITIES:
        try:
            ...
            # --- THE SYNC LOGIC ---
            # A. Delete existing record for this city to keep the table clean
            supabase.table("city_stats").delete().eq("city_name", city['name']).execute()
            
            # B. Insert the new data point
            response = supabase.table("city_stats").insert(data_to_insert).execute()
            
            print(f"✅ Successfully refreshed {city['name']}: {temp}°C, AQI: {aqi_500_score}")
